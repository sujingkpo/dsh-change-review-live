...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function LivePanel(props) {
			...
			// 范围筛选：默认只看「最新轮次」（最近有改动的那一轮），可切到「全部」
			...
			const scopeList = (scope === "latest" && scopeTurn > 0) ? latestList : allList;
			...
			// 空列表提示：区分「还在加载」「这一轮确实没有」「被筛选隐藏了」
			const emptyHint = (latestLoading && scope === "latest")
				? "加载当前轮次…"
				: (scope === "latest" && latestList.length === 0)
					? "本轮暂无变更"
					: "当前范围的文件都被降噪开关藏起来了（点右侧图标可查看）";
			// 标题行「最近变动」摘要数据 = 当前轮次的文件；这一轮还没有变更时**不显示**摘要
			// （刻意不回退全量：回退会让新一轮开始后标题栏继续显示上一轮的旧信息）
			const titleList = latestList;
			// 该轮问题：sections 已倒序，sections[0] 即该文件该轮最近一次修改
			const qOf = (f) => (f && f.sections && f.sections.length > 0 && f.sections[0].question) || '';
			// 标题栏聚合摘要：「最近变动 - 文件列表 + 问题」；数据取**当前轮次**
			// （titleList = latestList，本轮没有变更时不显示摘要）；文件最多显示 2 个
			...
			return React.createElement(React.Fragment, null,
				...
				React.createElement("div", { className: "dsdrv-livepanel-head" + (collapsed ? "" : " dsdrv-livepanel-head-open"), title: collapsed ? "展开审查列表" : "折叠审查列表", onClick: () => setCollapsed(!collapsed) },
					...
					React.createElement("span", { className: "drv-header-spacer" }),
					// 范围筛选：互斥两枚纯图标 + 数量（首行高亮 = 只看最新轮次；三行 = 全部），
					// 完整语义放 title / aria-label，窄面板也放得下
					React.createElement("button", {
						type: "button",
						className: "drv-btn dsdrv-livepanel-filter dsdrv-livepanel-filter-icon" + (scope === "latest" ? " dsdrv-livepanel-filter-on" : ""),
						title: "只看最新轮次（" + latestCount + " 个文件）",
						"aria-label": "只看最新轮次",
						onClick: (e) => { e.stopPropagation(); setScope("latest"); }
					},
					...
					React.createElement("button", {
						...
						title: "显示全部轮次（" + allCount + " 个文件）",
						"aria-label": "显示全部轮次",
						onClick: (e) => { e.stopPropagation(); setScope("all"); }
					},
						React.createElement("svg", { width: 12, height: 12, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round" },
							React.createElement("path", { d: "M4 6h16" }),
					...
					deletedCount > 0 ? React.createElement("button", {
						...
						React.createElement("span", { className: "dsdrv-livepanel-filter-n" }, deletedInScope + "/" + deletedCount)) : null,
					// 工作区外：图标 + 数量（默认隐藏）
					outsideCount > 0 ? React.createElement("button", {
						...
						React.createElement("span", { className: "dsdrv-livepanel-filter-n" }, outsideInScope + "/" + outsideCount)) : null,
